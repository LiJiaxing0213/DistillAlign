...
class BidirectionalDiffusionInferencePipeline(torch.nn.Module):
    def __init__(
        ...
        shared_model_name = model_kwargs.get("model_name", "Wan2.1-T2V-1.3B")
        model_root = model_kwargs.get("model_root", "wan_models")
        self.generator = WanDiffusionWrapper(
            **model_kwargs, is_causal=False) if generator is None else generator
        self.text_encoder = WanTextEncoder(
            shared_model_name, model_root) if text_encoder is None else text_encoder
        self.vae = WanVAEWrapper(
            shared_model_name, model_root) if vae is None else vae
